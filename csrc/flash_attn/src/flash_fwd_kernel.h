...
inline __device__ void compute_attn_1rowblock_splitkv(const Params &params, const int bidb, const int bidh, const int m_block, const int n_split_idx, const int num_n_splits) {
    ...
    //     + (n_block_max - 1) * kBlockN * params.v_row_stride + (bidh / params.h_h_k_ratio) * params.v_head_stride;
    const int64_t row_offset_k = binfo.k_offset_pg(params.k_batch_stride, params.k_row_stride, bidb_cache, n_block_max - 1, kBlockN)
        + (bidh / params.h_h_k_ratio) * params.k_head_stride;

    const int64_t row_offset_v = binfo.k_offset_pg(params.v_batch_stride, params.v_row_stride, bidb_cache, n_block_max - 1, kBlockN)
    ...
    // TODO(scv119): this might be wrong for valren q.
    const index_t row_offset_o = binfo.q_offset(params.o_batch_stride, params.o_row_stride, bidb)
        + m_block * kBlockM * params.o_row_stride + bidh * params.o_head_stride;
    const index_t row_offset_oaccum = (((n_split_idx * params.b + bidb) * params.h + bidh) * params.seqlen_q
                                         + m_block * kBlockM) * params.d_rounded;
    ...
    Tensor gLSEaccum = make_tensor(make_gmem_ptr(reinterpret_cast<ElementAccum *>(Split ? params.softmax_lseaccum_ptr : params.softmax_lse_ptr) + row_offset_lseaccum),
                                   Shape<Int<kBlockM>>{}, Stride<_1>{});
    // if (tidx == 0) { printf("row_offset_o = %d, row_offset_oaccum = %d, bidh = %d, gOaccum = %p\n", row_offset_o, row_offset_oaccum, bidh, gOaccum.data()); }
...
inline __device__ void combine_attn_seqk_parallel(const Params &params) {
    ...
    for (int m = 0; m < size<1>(rO); ++m) {
        ...
        if (idx < params.b * params.h * params.seqlen_q) {
            ...
            int batch_offset = batch_idx * params.o_batch_stride;
            // varlen
