...
inline __device__ void apply_mask(Tensor<Engine, Layout> &tensor, const int max_seqlen_k,
    ...
    DEBUG_PRINT("applying mask: max_seqlen_k = %d, offset = %d\n", max_seqlen_k, col_idx_offset_);
...
inline __device__ void apply_mask_local(Tensor<Engine, Layout> &tensor, const int col_idx_offset_,
    ...
    // tensor has shape (ncol=(2, MMA_M), nrow=(2, MMA_N))
    DEBUG_PRINT("apply_mask_local, col_idx_offset_=%d, max_seqlen_k=%d, row_idx_offset_=%d, max_seqlen_q=%d, warp_row_stride=%d, window_size_left=%d, window_size_right=%d\n",
