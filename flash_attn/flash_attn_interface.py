...
def flash_attn_with_page_attention(
    q,
    k_cache,
    v_cache,
    block_tables,
...
def flash_attn_varlen_with_page_attention(
    q,
    k_cache,
    v_cache,
    block_tables,
    cache_seqlens_q: torch.Tensor = None, 
    cache_seqlens_k: torch.Tensor = None,
    max_seqlen_q: int,
    max_seqlen_k: int,
    k=None,
    v=None,
    ...
    Arguments:
        ...
        rotary_cos [optional]: (seqlen_ro, rotary_dim / 2). If not None, we apply rotary embedding
            to k and q. Only applicable if k and v are passed in. rotary_dim must be divisible by 16.
        rotary_sin [optional]: (seqlen_ro, rotary_dim / 2). Similar to rotary_cos.
        cache_seqlens_q: (batch_size,), dtype torch.int32. The sequence lengths of the
            query.
        cache_seqlens_k: (batch_size,), dtype torch.int32. The sequence lengths of the
            KV cache.
        cache_batch_idx: (batch_size,), dtype torch.int32. The indices used to index into the KV cache.
            If None, we assume that the batch indices are [0, 1, 2, ..., batch_size - 1].
            If the indices are not distinct, and k and v are provided, the values updated in the cache
    ...
    #assert cache_batch_idx is None, "cache batch idx is not supported by page attention"
    assert k is None and v is None, "new k and v is not supported by page attention"
    assert isinstance(cache_seqlens_q, torch.Tensor), "cache_seqlens_q should be a tensor"
    assert isinstance(cache_seqlens_k, torch.Tensor), "cache_seqlens_k should be a tensor"
    maybe_contiguous = lambda x: x.contiguous() if x is not None and x.stride(-1) != 1 else x
    q, k, v = [maybe_contiguous(x) for x in (q, k, v)]
    if softmax_scale is None:
        softmax_scale = q.shape[-1] ** (-0.5)
    cache_batch_idx = maybe_contiguous(cache_batch_idx)
    out, softmax_lse = flash_attn_cuda.fwd_varlen_page_attention(
        q,
        k_cache,
        v_cache,
        block_tables,
